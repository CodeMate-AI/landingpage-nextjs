...
const nextJsStructure = [
  ...
  return(
    <div className='relative h-full w-full bg-zinc-900  rounded-xl flex overflow-hidden'>
       ...
       <div className='absolute h-[5%] w-full bottom-0 bg-zinc-800 rounded-b-xl z-50 flex  gap-5 items-center border-t-[0.1px] border-white border-opacity-10 justify-between px-3'>
...
<span className='opacity-70 text-sm mt-2'>page.tsx</span>
      </div>
     </div>
      <motion.div 
      ...
      <div className='relative flex flex-col gap-[0.37rem] font-mono w-full '>
        {comp1.map((e:any,idx:number)=>(
         ...
         <div key={idx} className='relative flex'>
          ...
          <span  className='w-full ml-2 cursor-text'>{e.code}</span>
                  <motion.img 
        animate={{opacity:[0,1]}}
     ...
     <div className='relative h-full  flex gap-3 py-2 w-full'>
      ...
      <span className='text-mono text-sm opacity-70'>8 FIXES</span>
      </motion.div>
